...
const ResidentProfile = () => {
  ...
  const discard = () => {
    setEditing(null);
    setPassword("");
    setName(localUser?.user?.full_name || "");
  };

  const handleUpdate = async () => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/resident/update-profile`,
        {
          method: "PATCH",
          headers: {
          ...
          body: JSON.stringify({
            user_id: localUser?.user?.user_id,
      ...
      const updated = { ...localUser, user: data.result };
